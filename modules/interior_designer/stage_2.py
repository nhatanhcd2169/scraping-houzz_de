...
import modules.interior_designer.const as const
...
def scrape(url: str):
    ...
    info['name'] = (
        info['business_name'] 
        if 'interior' not in info['business_name'].lower()
        else info['business_name'][:info['business_name'].lower().index('interior')].strip()
    )
    return info
